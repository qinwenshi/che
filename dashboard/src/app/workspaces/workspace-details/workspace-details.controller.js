...
export class WorkspaceDetailsCtrl {
   ...
   */
  constructor($rootScope, $route, $location, cheWorkspace, $mdDialog, cheNotification, ideSvc, $log, workspaceDetailsService, lodash, $timeout) {
  ...
  doUpdateWorkspace() {
    ...
    workspaceNewDetails.config.environments.forEach((env) => {
      if (env.name === workspaceNewDetails.config.defaultEnv) {
        env.machineConfigs.forEach((machine) => {
          if (machine.dev === true) {
